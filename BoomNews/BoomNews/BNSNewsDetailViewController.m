...
- (void)viewDidLoad {
	...
	if (self.newsModel.store) {
		[self._rightButton setImage:[UIImage imageNamed:@"starSelected"] forState:UIControlStateNormal];
	}else {
		[self._rightButton setImage:[UIImage imageNamed:@"star"] forState:UIControlStateNormal];
	}
...
- (void)collectAction:(UIButton *)sender {
    ...
    if (!self._rightButton.selected && !self.newsModel.store) {
        DataMessageBaseManaher *manager = [DataMessageBaseManaher shareDataBaseManager];
        [manager openDB];
        [manager createTable];
		self.newsModel.store = YES;
		...
		DataMessageBaseManaher *manager = [DataMessageBaseManaher shareDataBaseManager];
		[manager openDB];
		self.newsModel.store = NO;
		[manager deleteWithMessage:[self.newsModel title]];
		
        UIAlertView *collectView = [[UIAlertView alloc] initWithTitle:@"取消收藏" message:nil delegate:self cancelButtonTitle:@"确认" otherButtonTitles:nil];
        [collectView show];
		 self._rightButton.selected = NO;
		[self._rightButton setImage:[UIImage imageNamed:@"star"] forState:UIControlStateNormal];
...
- (void)loadImageView {
    ...
    self.bnsView = [[[BNSView alloc]initWithFrame:CGRectMake(0, 15, self.view.frame.size.width, self.view.frame.size.height - 64)] autorelease];
